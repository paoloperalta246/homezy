...
const Services = () => {
  ...
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        ...
        // Extract unique locations from service category only
    ...
    const fetchUserBookings = async () => {
      ...
      try {
        ...
        if (allServices.length > 0) {
          ...
          let suggestions = allServices.filter(listing => {
  ...
  return (
    <div className="font-sans bg-[#fefefe] min-h-screen">
      ...
      {user && suggestedServices.length > 0 && !isSearching && (
        <section className="max-w-screen-xl mx-auto mt-14 mb-14 sm:mb-20 px-4 sm:px-6 lg:px-6">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-500 text-sm sm:text-base mt-2">
              Based on your booking history, we think you'll love these services
            </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 justify-items-center">
            {suggestedServices.map((item) => renderCard(item))}
          </div>
        </section>
      )}
